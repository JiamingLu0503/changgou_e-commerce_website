...
public class SeckillGoodsController {
    ...
    /****
     * 根据时间菜单，seckillgoods id查询商品详情
     * @param time :时间区间
     * @param id :商品ID
     */
    @RequestMapping(value = "/one")
    public Result<SeckillGoods> one(String time, Long id) {
        SeckillGoods seckillGood = seckillGoodsService.one(time, id);
        return new Result<>(true, StatusCode.OK, "查询秒杀商品详情查询成功", seckillGood);
    }
    ...
    public Result<List<SeckillGoods>> list(String time) {
        ...
        return new Result<>(true, StatusCode.OK, "对应时间段秒杀商品集合查询成功", seckillGoods);
    }
    ...
    public Result<PageInfo> findPage(
            @RequestBody(required = false) @ApiParam(name = "SeckillGoods对象", value = "传入JSON数据", required = false)
                    SeckillGoods seckillGoods, @PathVariable int page, @PathVariable int size) {
    ...
    public Result<List<SeckillGoods>> findList(
            @RequestBody(required = false) @ApiParam(name = "SeckillGoods对象", value = "传入JSON数据", required = false)
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @PutMapping(value = "/{id}")
    public Result update(@RequestBody @ApiParam(name = "SeckillGoods对象", value = "传入JSON数据", required = false)
                                 SeckillGoods seckillGoods, @PathVariable Long id) {
        //设置主键值
    ...
    @PostMapping
    public Result add(@RequestBody @ApiParam(name = "SeckillGoods对象", value = "传入JSON数据", required = true)
                              SeckillGoods seckillGoods) {
